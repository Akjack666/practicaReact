...
export default class SaleSearch extends React.Component {
    constructor(props) {
        ...
        const result = JSON.parse(sessionStorage.getItem('currentUser')).tag;
            ...
            <div className={`sale-search container`}>
                <div className="row mb-3">
                    ...
                    <input name="price" type="number" onChange={this.handleSearch} className={`form-control col-1 ml-4`} placeholder={`Price`}/>
                    {
                        this.state.tags
                        &&
                        <select name="tag" value={this.state.search.tag ? this.state.search.tag : ""} onChange={this.handleSearch} className={`form-control col-2 ml-4`}>
